...
RAW_SHEETS    = ["fbS", "fbV",
                 ...
                 # 21.09: адмін перепідключив експорт форм після їх оновлення в Meta —
                 # ліди пішли в нові вкладки з назвою форми (+ дублі fb0/fbg/fb4/fbM).
                 # ПАМ'ЯТКА: після кожного редагування форми в Meta експорт створює нову
                 # вкладку — її треба додати сюди, інакше ад-сети/креативи без записів.
                 "20.08_Kyiv_form (v3)", "20.08_Kyiv_form (v4)", "20.08_Ternopil_form (v2)",
                 "fb0", "fbg", "fb4", "fbM", "fb3", "fb5", "fb6"]
...
def main():
    ...
    # rows is None = Meta не відповіла (протухлий токен тощо): денні ряди лишаються старі,
    # тому чесно позначаємо «stale» — сайт і бриф покажуть алерт замість тихого замерзання
    out["metaSource"] = "graph" if rows is not None else "graph-stale"
    out["metaToken"]  = meta_token_info()
