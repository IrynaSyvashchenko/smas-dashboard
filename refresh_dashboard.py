...
def is_booked(r):
    ...
    for sf in STATUS_FIELDS:
        ...
        # НЕ рахуємо: заперечення («не записалась», «отказ от записи», «без записи»)
        # та «уже запис…» — людина вже була записана через ІНШОГО таргетолога (правило Ірини)
        if "запис" in s and not any(n in s for n in ("не запис", "отказ", "без запис", "уже запис")):
            return True
    return False
