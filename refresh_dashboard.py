...
def _admin_mgr(name):
    ...
    if "карина" in n or "каріна" in n: return "Карина"
...
MANAGER_HIDE = ("Юлиана", "Вика", INST_MGR_PARIS, INST_MGR_PRAGUE)   # Вика схована 17.09 (не працює)
...
MANAGER_GID = {"Диана": "1178192251", "Таня": "1053387771",
               ...
               "Карина":   (DANIK_SS, "2128779218"),   # «Карина Ирина» (вкладка «Карина Вера» — чужа)
...
def _remap_new_row(r):
    ...
    if not str(r.get("created_time") or "").strip():
        ph = re.sub(r"\D", "", str(r.get("phone_number") or ""))[-9:]
        if ph in NODATE_PATCH:
            r["created_time"] = NODATE_PATCH[ph]
    return r

# Разова латка збою авто-заливки 05.09.2026 (лежала ~07:30 05.09 - 08:20 06.09 Праги):
# празькі ліди донесені в CRM вручну БЕЗ дати (редагувати таблицю Ірина не може) —
# дати відновлено з вивантаження Центру лідів (празький час переведено в пояс
# вкладок -05:00). Застосовується ЛИШЕ до рядків, де своєї дати немає.
NODATE_PATCH = {
    "973710646": "2026-09-05T03:53:00-05:00",  # Валентина Мокринська (Диана)
    "723500025": "2026-09-05T04:33:00-05:00",  # Bobina Vašek Triskovi (Диана)
    "705685026": "2026-09-05T04:58:00-05:00",  # Petronela Gabčová (Диана)
    "737181095": "2026-09-05T09:26:00-05:00",  # Iveta Turková (Диана)
    "704704445": "2026-09-05T01:57:00-05:00",  # Niki Filipova (Даша)
    "724662309": "2026-09-05T02:29:00-05:00",  # Jana Chlupáčková (Даша)
    "603586067": "2026-09-05T04:06:00-05:00",  # Jana Janichová (Даша)
    "736431454": "2026-09-05T05:55:00-05:00",  # Monika Pinky (Даша)
    "606375050": "2026-09-05T07:32:00-05:00",  # Denise (Даша)
    "605881837": "2026-09-05T07:46:00-05:00",  # Jana Smolkova (Даша)
    "723136200": "2026-09-05T08:31:00-05:00",  # Veronika Tymrova (Даша)
    "730858984": "2026-09-05T08:33:00-05:00",  # Petra Vichrová (Даша)
    "799502393": "2026-09-05T09:39:00-05:00",  # Jana Křížová (Даша)
}
...
def is_booked(r):
    ...
    for sf in _status_keys(r):
        ...
        if "отказ" in s or re.search(r"не\s*удобн", s):   # «не удобны даты для записи» = відмова (Мінськ 17.09)
            continue
        ...
        for n in ("не запис", "без запис", "уже запис", "для запис"):
            s = s.replace(n, "")
        if "запис" in s:
            return True
    return False
...
def fetch_raw_map(barca_camps=frozenset()):
    ...
    # Каріна: сира вкладка fb1 у тій самій таблиці (fbK — РК Вери, не читаємо)
    try:
        _ingest(fetch_sheet_rows(ss=DANIK_SS, gid="501615709"), "Карина", "danik:fb1")
    except Exception as e:
        print("  сира вкладка fb1 (Каріна) ->", str(e)[:60])
